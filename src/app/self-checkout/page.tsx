...
export default function SelfCheckoutPage() {
  ...
  async function handlePlaceOrder() {
    ...
    // Offline kiosk: queue the order locally and show a "syncing" done screen.
    // OfflineSync flushes it (idempotently) on reconnect; selfStatus then flips to
    // the real order number, or "see staff" if the table was taken meanwhile.
    if (OFFLINE_ENABLED && typeof navigator !== "undefined" && !navigator.onLine) {
      ...
      setQueuedId(id);
      setResult({ orderNumber: null, tableNumber: selectedTable.number, total: totals.total });
      setStep("done");
      return;
    }
